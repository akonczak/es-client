...
docker run --name ${CONTAINER_NAME} -d -p ${HTTP_PORT}:9200 -p ${TRANSPORT_PORT}:9300 -e "xpack.security.enabled=false" -e "discovery.type=single-node" docker.elastic.co/elasticsearch/elasticsearch:${ES_VERSION}
